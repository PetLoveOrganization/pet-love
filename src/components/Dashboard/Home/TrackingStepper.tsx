...
export const AdoptionStepper = ({ currentStatus = AdoptionRequestStatus.PENDING, petName }: StepperProps) => {
  return (
    <section className="bg-white p-6 rounded-xl shadow-xs">
      <h2 className="text-base md:text-lg font-semibold text-gray-800 mb-4">Follow-up: Request for {petName}</h2>

      <div className="relative flex justify-between items-center">
        <div className="absolute top-7 md:top-8.5 left-0 w-full h-1 bg-gray-100 z-0 rounded-full overflow-hidden">
          <div
        ...
        <div className="relative z-10 flex flex-col items-center gap-3">
          <div className="size-8 md:size-10 rounded-full bg-green-pet text-white flex items-center justify-center shadow-lg shadow-black/15">
            <Point className='size-3 md:size-4 text-black' />
          </div>
          <span className='text-xs md:text-base'>Sended</span>
        </div>

        <div className="relative z-10 flex flex-col items-center gap-3">
          <div className={`size-8 md:size-10 rounded-full flex items-center justify-center transition-all duration-300 text-black shadow-lg   ${
          ...
          <span className="text-xs md:text-base">Review</span>
        </div>

        <div className="relative z-10 flex flex-col items-center gap-3">
          <div className={`size-8 md:size-10 rounded-full flex items-center justify-center  transition-all duration-300 shadow-lg ${
            currentStatus === AdoptionRequestStatus.PENDING
            ...
            {currentStatus === AdoptionRequestStatus.PENDING && <span className="text-xs md:text-lg">3</span>}
